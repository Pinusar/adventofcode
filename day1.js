...
for (const line of lines) {
    ...
    for (const number of numbers) {
        ...
        if (firstIndex !== -1 && firstMax === null) {
            firstWord = number
            firstValue = map[number]
            firstMax = firstIndex
        }
        if (firstIndex !== -1 && firstMax !== null && firstIndex < firstMax) {
            firstWord = number
            firstValue = map[number]
            firstMax = firstIndex
        }
        ...
        let lastIndex = line.lastIndexOf(number);
        if (lastIndex !== -1 && lastMax === null) {
            lastWord = number
            lastValue = map[number]
            lastMax = lastIndex
        }
        if (lastIndex !== -1 && lastMax !== null && lastIndex > lastMax) {
            lastWord = number
            lastValue = map[number]
            lastMax = lastIndex
        }
    ...
    for (let i = 0; i < line.length; i++) {
        if (line[i] >= '0' && line[i] <= '9') {
            if (first === '' || (firstMax != null && i < firstMax)) {
                first = line[i]
                firstMax = i;
            }
            if (last === '' || (lastMax != null && i > lastMax)) {
                last = line[i]
                lastMax = i;
            }
        }
