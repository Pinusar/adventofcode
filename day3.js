...
for (const num of numbers) {
    ...
    let upperRow;
    if (num.start.row > 0) {
        upperRow = lines[num.start.row - 1];
    }
    let lowerRow;
    if (num.start.row < lines.length - 1) {
        lowerRow = lines[num.start.row + 1];
    }
    let isOnLeftEdge = num.start.position === 0;
    const leftStart = isOnLeftEdge ? 0 : num.start.position - 1;
    let isOnRightEdge = num.end.position === lines[0].length - 1;
    const rightEnd = isOnRightEdge ? num.end.position : num.end.position + 1;

    if (upperRow) {
        for (let j = leftStart; j <= rightEnd; j++) {
            if (isASymbol(upperRow[j])) {
                isPartNumber = true;
            }
        }
    }
    if (lowerRow) {
        for (let j = leftStart; j <= rightEnd; j++) {
            if (isASymbol(lowerRow[j])) {
                isPartNumber = true;
            }
        }
    }
    let numLine = lines[num.start.row];
    if (!isOnLeftEdge) {
        if (isASymbol(numLine[num.start.position - 1])) {
            isPartNumber = true;
        }
    }
    if (!isOnRightEdge) {
        if (isASymbol(numLine[num.end.position + 1])) {
            isPartNumber = true;
        }
    }
    if (isPartNumber) {
        ...
        console.log('not a part number: ', num)
    }
...
function getRealInput() {
    return `416.........................559...............417...............785.......900.......284...........503...796....992..........................
